...
states_dict = dict(zip(estados, siglas))
...
crops_df['UF_sigla'] = [states_dict[v] for v in crops_df['UF']]
...
columns_dict = {column:k for k, column in enumerate(crops_df.columns)}
...
for crop in crops:
    ...
    if read_shape:
        ...
        geodata.loc[geodata['name_muni']] = \
            geodata['name_muni'].str.lower().apply(unidecode)\
                .str.replace('-',' ')

        culturas_df.loc[culturas_df[columns_dict['Municipio']] = \
            culturas_df[columns_dict['Municipio']].str.lower().apply(unidecode)\
                .str.replace('-',' ')
    ...
    col = columns_dict['Quantidade produzida (Toneladas)']

    column_path = shape_path + "/" + col.replace("/", "_")
    ...
    if culturas_df[col].count() > 5:
        ...
        # cultura_geodf = culturas_df.loc[(culturas_df['Nivel']==4), ['geometry', 'Municipio', col]].fillna(0)
